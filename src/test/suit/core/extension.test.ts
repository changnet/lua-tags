...
import { activateExtension, testWorkspaceSymbol, testDocumentSymbol, testLuaCheck, resolveFixture } from '../../helper';
...
suite('Extension Test Suite', () => {
	...
	test('test fuzz workspace symbol', async () => {
		...
		const uri0 = vscode.Uri.file(path.join(fixturePath,
			"conf", "battle_conf.lua"));
		const uri1 = vscode.Uri.file(path.join(fixturePath,
			"conf", "monster_conf.lua"));
		const uri2 = vscode.Uri.file(path.join(fixturePath,
			"conf", "skill_conf.lua"));
